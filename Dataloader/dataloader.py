...
class Dataloader:
    def __init__(self, opt, phase):
        ...
        for aSub in sorted(listdir(self.phase_root)):
            if aSub.endswith('.mat'):
                sub_root = join(self.phase_root, aSub)
                flist.append(sub_root)
    ...
    def read_mat(filename):
        ...
        data = mat['Spec2D']
        slice = np.zeros((320,320))
        slice = data[100:420, 100:420]
        return slice

    @staticmethod
    def read_test_mat(filename):
        """
        Read data from .mat file
        :param filename: the name of the file
        :return: 3D (HWC) output numpy array
        ...
        # read 1st 5qi data
        mat = sio.loadmat(filename, verify_compressed_data_integrity=False)
        data = mat['Spec2DNoise'][:,:,:]
        slice = np.zeros((data.shape[0],320,320))
        slice[:,30:286, :] = data
        return slice
    ...
    def make_mask_X(inp, R):
        ...
        nACS = round(nX / (R ** 2))
        ACS_s = round((nX - nACS) / 2)
        ACS_e = ACS_s + nACS
        mask[:, ACS_s:ACS_e] = 1

        nSamples = int(nX / R)
        r = np.floor(np.random.normal(nX / 2, 70, nSamples))
        r = np.clip(r.astype(int), 0, nX - 1)
        mask[:, r.tolist()] = 1
        return mask
    ...
    def getBatch_magnitude_test_2(self, idx):
        """
        Make and return batch data (for magnitude image, inference)
        :param idx: integer, the index of the file list
        :return: 4D (BHWC) output numpy array
        ...
        aInput_D_C = self.read_test_mat(batch_D)  # magnitude image with motion artifact

        size_input = [self.N, self.nY, self.nX, self.nC*aInput_D_C.shape[0]]

        Input_D = np.empty(size_input, dtype=np.float32)
        Scale_D = np.empty(size_input, dtype=np.float32)

        for iC in range(aInput_D_C.shape[0]):
            aInput_D = aInput_D_C[iC, :, :]
            # if iC==30:
                # plt.figure(1)
                # plt.imshow(np.squeeze(aInput_D), cmap="gray")
            for iB in range(self.N):
                aMask_D = self.make_mask(aInput_D, self.R)

                # plt.figure(iB+2,figsize=(1200,1200),dpi=1800)
                # plt.subplot(1,2,1)
                # plt.imshow(np.squeeze(aMask_D), cmap="gray")
                k_orig_D = np.fft.fftshift(np.fft.fft2(aInput_D))
                k_down_D = k_orig_D * aMask_D
                aInput_D = np.abs(np.fft.ifft2(np.fft.ifftshift(k_down_D)))
                ...
                Input_D[iB, :, :, iC] = aInput_D
                Scale_D[iB, :, :, iC] = np.tile(aScale_D, [self.nY, self.nX])
    ...
    def getBatch_complex(self, start, end):
        ...
        for iB in range(len(batch_F)):
            ...
            aMask_F = self.make_mask_X(aInput_F, self.R)
            aMask_D = self.make_mask_X(aInput_D, self.R)
