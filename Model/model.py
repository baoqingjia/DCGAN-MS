...
class MR_motion_magnitude:
    ...
    def _build_model(self):
        ...
        with tf.device(self.GPU):
            ...
            # self.G_F2D = Downsampling(self.opt.img_mode, name='Gen_F2D')
    ...
    def train(self, dataloader):
        ...
        for epoch in tqdm(range(epoch_start, self.num_epoch), desc='Epoch', total=self.num_epoch, initial=epoch_start):
            ...
            for step in tqdm(range(self.num_step_train), desc='Step'):
                ...
                # real_F, mask_F, real_D, mask_D = dataloader.getBatch_magnitude_nC(step * self.batch_size, (step + 1) * self.batch_size)
    ...
    def test(self, dataloader):
        # latest_ckpt = tf.train.latest_checkpoint(self.ckpt_dir)
        # self.saver.restore(self.sess, latest_ckpt)
        best_ckpt = 'Results/MR_motion_reduction/ckpt_dir/model.ckpt-140'
        self.saver.restore(self.sess, best_ckpt)

        save_path = join(self.save_path, self.experiment_name, 'test_N={}'.format(self.N))
        # save_D_path = join(self.save_path, self.experiment_name, 'test_real_D')

        if not isdir(save_path):
            makedirs(save_path)

        # if not isdir(save_D_path):
        #     makedirs(save_D_path)

        for step in tqdm(range(self.num_step_test), desc='Step'):
            real_D, scale_D = dataloader.getBatch_magnitude_test(step)
            # real_D, scale_D = dataloader.getBatch_magnitude_nC_test(step)
            ...
            fname = dataloader.flist_D[step].split('test/')[1]

            test_output = {'data': test_output[30:290, :]}
            # test_real_D = {'data': real_D[5, :, :, 0]}

            # sub_save_path = join(save_path, subname)
            if not isdir(save_path):
                makedirs(save_path)

            sio.savemat(join(save_path, fname), test_output)
            # sio.savemat(join(save_D_path, fname), test_real_D)

    def test_2(self, dataloader):
        latest_ckpt = tf.train.latest_checkpoint(self.ckpt_dir)
        self.saver.restore(self.sess, latest_ckpt)
        # best_ckpt = 'Results/MR_motion_reduction/ckpt_dir/model.ckpt-140'
        # self.saver.restore(self.sess, best_ckpt)

        # save_path = join(self.save_path, self.experiment_name, 'test1_N={}'.format(self.N))
        save_path = join(self.save_path, self.experiment_name, 'test_9_6')
        # save_D_path = join(self.save_path, self.experiment_name, 'test_real_D')
        ...
        for step in tqdm(range(self.num_step_test), desc='Step'):
            real_D_C, scale_D_C = dataloader.getBatch_magnitude_test_2(step)

            test_output_C = np.zeros((real_D_C.shape[3], 256, 320))

            for iC in range(real_D_C.shape[3]):

                real_D = np.expand_dims(real_D_C[:, :, :, iC], axis=-1)
                scale_D = np.expand_dims(scale_D_C[:, :, :, iC], axis=-1)
                feed_dict = {self.real_D: real_D}
                test_output = np.squeeze(self.sess.run(self.fake_F, feed_dict) * scale_D)
                test_output = np.mean(test_output, axis=0)
                test_output_C[iC, :, :] = test_output[30:286, :]

            # plt.figure(2)
            # plt.imshow(np.squeeze(test_output_C[30,:,:]), cmap="gray")

            # subpath = dataloader.flist_D[step].split('test/')[1]
            # subname, fname = subpath.split('/')
            fname = dataloader.flist_D[step].split('test_motion_artifact/')[1]

            test_output_C = {'data': test_output_C}

            if not isdir(save_path):
                makedirs(save_path)

            sio.savemat(join(save_path, fname), test_output_C)
...
class MR_motion_complex:
    ...
    def train(self, dataloader):
        ...
        for epoch in tqdm(range(epoch_start, self.num_epoch), desc='Epoch', total=self.num_epoch, initial=epoch_start):
            ...
            for step in tqdm(range(self.num_step_train), desc='Step'):
                ...
                if step % disp_step == 0:
                    # for i in range(3):  # gan updata 3 times
                    fake_F, _, G_fake_gan_loss, cycle_F_loss, cycle_D_loss, G_sum, img_sum = self.sess.run(out_argmG, feed_dictG)
                    self.writer.add_summary(G_sum, epoch * self.disp_div_N + disp_cnt)
                    self.writer.add_summary(img_sum, epoch * self.disp_div_N + disp_cnt)
                else:
    ...
    def test(self, dataloader):
        ...
        for step in tqdm(range(self.num_step_test), desc='Step'):
            ...
            test_real_D = {'data': real_D}

            # sub_save_path = join(save_path, fname)
            if not isdir(save_path):
                makedirs(save_path)
